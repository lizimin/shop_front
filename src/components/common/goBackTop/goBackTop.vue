<template>
  <div :style="backTopContainerStyle" class="cpts-gobacktop" v-show="show" @click="startScrollToTop">
    <slot>
...
export default {
  props: {
    threshold: {
      // 可滚动控件滚动偏移多少后才显示回到顶部组件，必须大于 100。默认值为 150
      type: Number,
      required: false,
      default: 150,
      validator(val) {
    ...
    animationTime: {
      // 滚动到顶部的动画时间，单位为毫秒，100 到 200 之间。默认值为 150
      type: Number,
      required: false,
      default: 150,
      validator(val) {
    ...
    imgUrl: {
      // 滚动到顶部控件为图片模式时图片的 url，如果设置了该属性，则会忽略 svg 开头的相关属性。默认值为 null
      type: String,
      required: false,
      default: null
    },
    imgCss: {
      // 滚动到顶部控件为图片模式时图片的样式，如果设置了该属性，则会忽略 svg 开头的相关属性。默认值为 null
      type: String,
      required: false,
      default: null
    },
    svgMajorColor: {
      // 滚动到顶部控件为 SVG 模式时的主要颜色。默认值为 #bfbfbf
      type: String,
      required: false,
      default: "#bfbfbf"
    },
    svgMinorColor: {
      // 滚动到顶部控件为 SVG 模式时的次要颜色。默认值为 #bfbfbf
      type: String,
      required: false,
      default: "#bfbfbf"
    },
    svgType: {
      // SVG 模式的样式类型，包括 circle_border_arrow、circle_fill_arrow、rocket_smoke、rocket。默认值为 rocket
      type: String,
      required: false,
      default: "rocket"
    },
    right: {
      // 滚动到顶部控件右边缘离浏览器右边缘的距离，单位为 px，但是开发者只需要写数字就行。默认值为 30
      type: Number,
      required: false,
      default: 30
    },
    bottom: {
      // 滚动到顶部控件下边缘离浏览器下边缘的距离，单位为 px，但是开发者只需要写数字就行。默认值为 60
      type: Number,
      required: false,
      default: 60
    },
    width: {
      // 滚动到顶部控件宽度，单位为 px，但是开发者只需要写数字就行。默认值为 48
      type: Number,
      required: false,
      default: 48
    },
    height: {
      // 滚动到顶部控件高度，单位为 px，但是开发者只需要写数字就行。默认值为 48
      type: Number,
      required: false,
      default: 48
    }
  ...
  watch: {
    $route(to, from) {
      // if (!/goods/.test(to.path) || !/goods/.test(from.path)) {
      ...
      setTimeout(that.addScrollListener, 1000);
    }
  },
  methods: {
    clearTimer() {
    ...
    addScrollListener() {
      this.removeScrollListener();
      this.scrollableElement = document.getElementsByClassName(
        "go-back-top"
      )[0];
      if (this.scrollableElement) {
        ...
        this.addScrollListenerTimeout = setTimeout(
